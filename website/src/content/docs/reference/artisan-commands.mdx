...
| `--force` | Overwrite existing config and workflow files, skipping all confirmation prompts |
...
2. Auto-detects env variables from `.env.example`:
   - **Secret-backed** (DB_DATABASE, DB_USERNAME, DB_PASSWORD, MAIL_USERNAME, MAIL_PASSWORD, REDIS_PASSWORD, AWS_*) — presented via multiselect
   - **Static** (MAIL_SCHEME, MAIL_MAILER, SESSION_DRIVER, LARAVEL_PDF_DRIVER, any project-specific key with a non-placeholder value) — presented via multiselect, then each value can be edited
3. Prompts for build env vars, custom commands, seeders, Slack notifications, and envaudit
...
- `%%SEEDERS%%` — first-deploy seeder commands from `netsons-deploy.json` (or config fallback)
...
If `.github/workflows/deploy.yml` already exists, the installer will prompt you to confirm overwriting it. In non-interactive mode, the workflow is preserved unless `--force` is used.
...
- **Interactive mode** — ask before overwriting existing config and workflow files
- **Non-interactive mode** — skip overwriting existing files but still generate missing ones
