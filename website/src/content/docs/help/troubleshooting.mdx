...
The shared `.env` is created from `.env.example` on first deploy. SSH in and configure it:

```bash
ssh -p 65100 user@server
cd ~/public_html/current
/usr/local/bin/ea-php84 artisan key:generate
```

Then edit `~/public_html/shared/.env` with your database and other credentials.
