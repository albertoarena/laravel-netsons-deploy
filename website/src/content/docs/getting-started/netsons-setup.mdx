...
For private repos, the SSH key used for Netsons access must also be registered as a deploy key on GitHub. The workflow handles SSH agent forwarding and GitHub host key setup automatically. See [Private Repository Setup](/laravel-netsons-deploy/getting-started/github-secrets/#private-repository-setup-git-strategy) for details.
