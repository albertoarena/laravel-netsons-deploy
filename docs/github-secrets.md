...
If your repository is private, the SSH key used for deployment must also have read access to the GitHub repository. The workflow uses SSH agent forwarding to pass the runner's key through to the Netsons server when cloning.
...
> **Note:** The same private key must authenticate to both Netsons (SSH) and GitHub (git clone). If you use separate keys, you need a more complex SSH config — using one key for both is simpler.
